...
_STANDARD_DAILY_HOURS = 9.0


class TypeBTransformationStrategy(BaseTransformationStrategy):
    """Transformation strategy for Monthly Card (TYPE_B) reports with overtime breakdown."""

    def transform_row(self, row: AttendanceRow, seed: int) -> AttendanceRow:
        result = copy.copy(row)

        # Use slightly different jitter pattern than TYPE_A for variety
        jitter_entry = seed % 5
        jitter_exit = -(seed % 6)
        ...
        result.total_hours = self._compute_hours(
            result.entry_time, result.exit_time, row.total_hours
        )
        result.overtime_125_hours = self._compute_overtime_125(result.total_hours)

        logger.debug(
            f"TYPE_B transform [{row.date}]: "
            f"{row.entry_time}->{result.entry_time}, "
            f"{row.exit_time}->{result.exit_time}, "
            f"total={result.total_hours}, 125%={result.overtime_125_hours}"
        )
        return result
    ...
    def _compute_hours(self, entry: str, exit_t: str, fallback: float) -> float:
        try:
    ...
    def _compute_overtime_125(self, total_hours: float) -> float:
        overtime = max(0.0, total_hours - _STANDARD_DAILY_HOURS)
        return round(min(overtime, 2.0), 2)
